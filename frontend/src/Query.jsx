...
export default function Query({ token, activeSessionId, setActiveSessionId, setSessions }) {
  ...
  useEffect(() => {
    if (activeSessionId) {
      ...
      axios.get(`${API_URL}/chats`, { params: { token, session_id: activeSessionId } })
        .then(res => {
          if (res.data.chats) {
            const history = [];
  ...
  const ask = async () => {
    ...
    if (!currentSession) {
      ...
      if (setActiveSessionId) setActiveSessionId(currentSession);
      isNewSession = true;
    }
