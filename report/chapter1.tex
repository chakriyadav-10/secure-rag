...
\begin{enumerate}
    ...
    \item To develop a \textbf{context-aware PII detection engine} using a 25-character lookbehind algorithm specific to Indian financial document formats.
    ...
    \item To enforce \textbf{owner-based multi-tenant document isolation} at the Pinecone vector database query layer.
    \item To implement \textbf{Role-Based Access Control (RBAC)} with three-tier provisioning: Master Admin, Admin (Manager), and Customer.
\end{enumerate}

\section{Scope of the Project}

The system is designed as a \textbf{Secure Enterprise Banking RAG} application targeting internal bank operations. The scope includes:

\begin{itemize}
    \item A FastAPI-based backend deployed on Render with MongoDB Atlas and Pinecone as cloud databases.
    \item A React/Vite-based frontend deployed on Vercel with role-specific dashboards.
