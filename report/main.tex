...
The architecture employs a robust asynchronous backend service layer, a responsive client interface, a distributed NoSQL database for structured data persistence, a vector database for similarity search, and an embedding API for document vectorization. Multi-tenant document isolation is enforced at the vector database query layer through \texttt{owner\_id} metadata filtering. Experimental results demonstrate that the system successfully neutralizes eleven distinct attack categories while maintaining retrieval accuracy above 95\%.
