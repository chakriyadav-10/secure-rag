...
\section{Core Models and Frameworks Utilized}

Table~\ref{tab:frameworks} lists all models, frameworks, and cloud services integrated into the system.

\begin{table}[H]
\centering
\caption{Core models and frameworks}
...
Embedding Model & Gemini text-embedding-004 & 3072-dim document vectorization \\
LLM (Generation) & Gemini Flash Lite & Context-grounded answer generation \\
Vector Database & Pinecone (Serverless) & Cosine similarity search \\
Document Database & MongoDB Atlas & Users, chats, PII mappings \\
Password Hashing & Bcrypt (12 rounds) & One-way password storage \\
Token Management & PyJWT + HMAC-SHA256 & Stateless authentication \\
Encryption & AES-256-CBC (cryptography lib) & PII mapping encryption \\
PDF Parsing & PyPDF2 & Text extraction from PDFs \\
\bottomrule
\end{tabular}
\end{table}

\section{SIEM Audit Logs Integration}

Security Information and Event Management (SIEM) audit logging provides tamper-evident records of all security-relevant events across the system.

\subsection{Audit Event Model}

Each security event is captured with the following structured schema:

\begin{lstlisting}[caption={SIEM audit log event schema}, label={lst:siem}]
{
  ...
  "details": "Prompt injection detected: ignore previous instructions",
  "action_taken": "User blocked, document rejected"
}
\end{lstlisting}

\subsection{Logging Points in the Pipeline}

\begin{table}[H]
\centering
\caption{SIEM audit logging points}
\label{tab:siem_points}
\begin{tabular}{lll}
\toprule
\textbf{Event} & \textbf{Trigger} & \textbf{Severity} \\
\midrule
USER\_LOGIN & Successful authentication & INFO \\
LOGIN\_FAILED & Invalid credentials & WARNING \\
THREAT\_BLOCKED & Regex threat detection match & HIGH \\
PII\_DETECTED & PII entity found in document & INFO \\
DOC\_UPLOADED & Document successfully processed & INFO \\
RATE\_LIMIT\_HIT & User exceeded request quota & WARNING \\
USER\_BLOCKED & Persistent block applied & CRITICAL \\
MANAGER\_CREATED & Master Admin created admin & INFO \\
\bottomrule
\end{tabular}
\end{table}
...
The SIEM integration provides:
\begin{itemize}
    \item \textbf{Incident Traceability}: Every blocked attack is timestamped with the user identity, IP address, and the exact payload that triggered the block.
    \item \textbf{Forensic Analysis}: Security teams can reconstruct attack timelines from the audit trail.
    \item \textbf{Regulatory Compliance}: The logging structure aligns with SOC~2 Type~II and ISO~27001 audit requirements for financial services.
    \item \textbf{Real-Time Dashboard}: The Master Admin can view all security events through the SIEM Audit Logs panel in the frontend.
\end{itemize}

\section{Engineering Considerations and Deployment Notes}

\subsection{Deployment Architecture}

The system is deployed across three cloud platforms:
...
\begin{tabular}{llll}
\toprule
\textbf{Component} & \textbf{Platform} & \textbf{URL Pattern} & \textbf{Notes} \\
\midrule
Frontend & Vercel & \texttt{*.vercel.app} & Auto-deploys from GitHub \\
Backend & Render & \texttt{*.onrender.com} & Docker-based, auto-scaling \\
MongoDB & Atlas & \texttt{*.mongodb.net} & M0 free tier, encrypted at rest \\
Pinecone & Serverless & \texttt{*.pinecone.io} & us-east-1, cosine metric \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Environment Variables and Key Management}

All sensitive credentials are stored as environment variables on the deployment platform, never in source code:

\begin{itemize}
    \item \texttt{SECRET\_KEY}: JWT signing and AES key derivation seed
    \item \texttt{AES\_ENCRYPTION\_KEY}: Fallback encryption key for PII mappings
    \item \texttt{MONGO\_URI}: MongoDB Atlas connection string with TLS
    \item \texttt{PINECONE\_API\_KEY}: Pinecone vector database authentication
    \item \texttt{GEMINI\_API\_KEY}: Google Gemini Embedding and LLM API
    \item \texttt{MASTER\_ADMIN\_USER / PASS}: Infrastructure-level admin credentials
\end{itemize}

\subsection{CORS and API Security}

Cross-Origin Resource Sharing (CORS) is configured to allow only the Vercel frontend domain, preventing unauthorized API access from external origins. All API endpoints require JWT bearer tokens except \texttt{/register}, \texttt{/login}, and \texttt{/check-username}.
