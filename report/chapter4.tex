...
The Security Validation Engine scans all input text against two categories of attack patterns using precompiled regular expressions.
...
The mapping between tokens and original values is encrypted before storage in a document database using AES-256 in CBC mode.
...
\textbf{Storage format} in the document database:
\begin{equation}
...
Each token ID is mapped to a $d$-dimensional vector (where $d = 3072$ for the embedding model) through an embedding matrix $\mathbf{E} \in \mathbb{R}^{|V| \times d}$:
...
The transformer processes the token embeddings using the \textbf{Scaled Dot-Product Self-Attention} mechanism:
...
\begin{equation}
\mathbf{v} = \text{EmbedModel}(\text{``Account Number: ACCOUNT\_f7ce ...''})\in \mathbb{R}^{3072}
\end{equation}
...
\begin{table}[H]
\centering
\caption{Differential privacy and system parameters}
\label{tab:dp_params}
\begin{tabular}{ccl}
\toprule
...
Dimensions & $d$ & 3072 (Embedding size) \\
\bottomrule
\end{tabular}
\end{table}
